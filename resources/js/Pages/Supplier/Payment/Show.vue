                                ...
                                <div class="d-flex flex-column gap-7 gap-md-10">
                                    ...
                                    <div class="d-flex justify-content-between" style="margin-bottom: -20px !important;">
                                        <div>
                                         ...
                                         <div>
                                            <span class="fw-bold fs-7">{{ $t('label.invoiceNumber') }}:</span> {{ props.payment?.invoice_number }}
                                        </div>
                                    </div>
                                    <!--begin::Message-->
                                    <!--begin::Separator-->
                                        ...
                                        <div class="table-responsive mb-9" style="border-color: #9291914f !important">
                                            <table class="table table-bordered fs-6 gy-3 mb-0" style="border-color: #9291914f !important; width: 100%;">
                                                ...
                                                <tbody class="fw-semibold text-gray-600">
                                                    ...
                                                    <tr >
                                                        <td></td>
                                                        <td>{{ $t('payment.label.total') }}</td>
                                                        <td class="text-end">{{ props.payment?.amount }}</td>
                                                    </tr>
                                                    <tr>
                                                      <td class="fw-bold fs-7">{{ $t('buyer.label.totalTransaction') }}: {{ props.supplierSummary?.total_transaction }}</td>
                                                      <td class="fw-bold fs-7">{{ $t('buyer.label.totalPayment') }}: {{ props.supplierSummary?.total_payment }}</td>
                                                      <td class="fw-bold fs-7 text-end">{{ $t('buyer.label.totalDue') }}: {{ props.supplierSummary?.total_due }}</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
...
const printInvoice = () => {
    ...
    document.title = `Supplier Payment Report ${invoiceNumber}`;
    window.print();
};
