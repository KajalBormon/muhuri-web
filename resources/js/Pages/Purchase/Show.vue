                                        ...
                                        <div class="w-100">
                                            <table class="table table-bordered w-100 align-middle table-row-dashed fs-6 gy-5 mb-0">
                                                ...
                                                <thead class="fs-6 fw-semibold text-gray-600">
                                                    <tr style="border-color: #9291914f">
                                                        ...
                                                        <th class="text-end w-10">
                                                            {{ props.purchase?.purchase_type === 'other' ? $t('label.quantity') : $t('sale.label.quantity') }}
                                                        </th>
                                                ...
                                                <tbody class="fw-semibold text-gray-600">
                                                    <tr v-for="purchaseItem in props.purchaseItems" :key="purchaseItem.id" style="border-color: #9291914f">
                                                       <td class="w-10">{{ brands(purchaseItem) ? brands(purchaseItem) : '' }} {{ `(${items(purchaseItem)})` }}</td>
                                                    ...
                                                    <tr style="border-color: #9291914f">
                                                        <td :colspan="props.purchase?.purchase_type === 'other'? 2 : 3"></td>
                                                        <td class="text-end" v-if="props.purchase?.purchase_type !=='other'">{{ $t('sale.label.quantity') }}</td>
                                                        <td class="text-end" v-if="props.purchase?.purchase_type !=='other'">{{ totalQuantity }}</td>
...
const totalQuantity = computed(() => {
    return Math.round((props?.purchaseItems ?? []).reduce((sum, item) => sum + (item.quantity || 0), 0));
});

const printInvoice = () => {
    const invoiceNumber = props.purchase?.invoice_number || 'invoice';
    document.title = `Purchase Report ${invoiceNumber}`;
    window.print();
};
