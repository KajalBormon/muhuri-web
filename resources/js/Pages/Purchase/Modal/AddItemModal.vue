...
watch(() => formData.total_box, (newValue: number | null | undefined) => {
    const parsed = typeof newValue === 'number' ? newValue : parseFloat((newValue ?? 0).toString());

    if (!isNaN(parsed) && parsed > 0) {
        isTotalBoxEntered.value = true;
        formData.total_poly = parsed * 2;
    } else {
        isTotalBoxEntered.value = false;
        formData.total_poly = 1;
    }
});

watch(() => formData.total_poly, (newValue: number | null | undefined) => {
    if (!isTotalBoxEntered.value) {
        const mir = formData.mir ?? 0;
        const poly = parseFloat(String(newValue)) || 0;
        formData.quantity = Math.max(mir * poly, 0);
    }
});
