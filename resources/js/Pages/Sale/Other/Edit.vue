                                        ...
                                        <tr v-for="(sale, index) in tableData" :key="sale.id">
                                            ...
                                            <td class="text-end">
                                                <div class="d-flex align-items-center justify-content-end">
                                                    ...
                                                    />
                                                    <DeleteConfirmationButton v-if="checkPermission('can-delete-other-sale')" iconClass="fs-3" title="Sale Item"  confirmRoute="sales.saleItems.destroy" :saleId="props?.sale?.id" :saleItem="sale" :messageTitle="`${$t('confirmation.saleItem.delete', { saleItem: sale.item_name})}`"/>
                                                </div>
                                            </td>
                                        </tr>
                                        <!-- Total Amount -->
...
onMounted(() => {
    if (props.sale) {
        ...
        tableData.value = props.sale?.sale_items.map((sale: any) => {
            ...
            const totalPrice = Math.round((sale.quantity * sale.unit_price) / itemUnitValue);
            return {
                id: sale.id,
                item_id: itemName?.id ?? 0,
                item_name: itemName?.name || '',
                unit_price: itemUnit ? sale.unit_price : sale.total_price,
                quantity: itemUnit ? sale.quantity : 1,
                total_price: itemUnit ? totalPrice : sale.total_price,
                note: sale.note
            };
        });
    }
