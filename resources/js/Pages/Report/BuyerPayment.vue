    ...
    <AuthenticatedLayout :breadcrumbs="props?.breadcrumbs" :pageTitle="props?.pageTitle">
        <div class="card pt-5">
            ...
            <div id="invoice-content">
                <h5 class="fs-3 text-center">{{ $t('report.label.fromDate', { date: formData.from_date }) }} - {{ $t('report.label.toDate', { date: formData.to_date, title: t('sidebarMenu.report.buyerPayment') }) }}</h5>
            </div>
                ...
                <div class="card-title">
                    ...
                    <div class="d-flex align-items-center position-relative my-1">
                        ...
                        <input
                            ...
                            :placeholder="$t('report.placeholder.searchBuyerPayments')"
                        />
                    </div>
                    <!--end::Search-->
                </div>
            ...
            <div class="card-body pt-0" id="invoice-content">
                <Datatable
                    class="table"
                    @on-sort="sortData"
                    :data="computedBuyerPaymentTable"
                    :header="tableHeader"
                ...
                >
                    ...
                    <template v-slot:invoice_number="{ row: buyerPayment }">
                        {{ buyerPayment.invoice_number }}
                    </template>

                    <template v-slot:buyer="{ row: buyerPayment }">
                        {{ buyerPayment.buyer }}
                    </template>

                    <template v-slot:payment_date="{ row: buyerPayment }">
                        <span v-if="!buyerPayment.is_total_row">{{ formatDate(buyerPayment.payment_date) }}</span>
                        <span v-else></span>
                    </template>

                    <template v-slot:payment_method="{ row: buyerPayment }">
                        <span v-if="!buyerPayment.is_total_row">{{ buyerPayment.payment_method }}</span>
                        <strong v-else class="text-end">{{ $t('label.total') }}</strong>
                    </template>

                    <template v-slot:amount="{ row: buyerPayment }">
                        <strong v-if="buyerPayment.is_total_row">{{ buyerPayment.amount }}</strong>
                        <span v-else>{{ buyerPayment.amount }}</span>
                    </template>
                </Datatable>
            </div>
        </div>
...
const initData = ref<IBuyerPayment[]>([]);

onMounted(() => {
    if(props.buyerPayments){
        initData.value = props.buyerPayments.map((buyerPayment: any)=>({
            id: buyerPayment.id,
            buyer: buyerPayment.buyer?.name,
            invoice_number: buyerPayment.invoice_number,
...
//Calculate Summation
const getTotal = (field: keyof IBuyerPayment) => {
  return tableData.value.reduce((sum, row) => sum + Number(row[field] || 0), 0);
};

const computedBuyerPaymentTable = computed(() => {
...
const applyFilter = () => {
    Inertia.replace(route('reports.buyerPayment'), {
        preserveScroll: true,
        preserveState: true,
        data: {
...
const printInvoice = () => {
    document.title = `Buyer Payment Report ${formData.from_date} to ${formData.to_date}`;
    window.print();
};
</script>
