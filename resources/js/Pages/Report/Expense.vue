    ...
    <AuthenticatedLayout :breadcrumbs="props?.breadcrumbs" :pageTitle="props?.pageTitle">
        <div class="card pt-5">
            ...
            <div id="invoice-content">
                <h5 class="fs-3 text-center">{{ $t('report.label.fromDate', { date: formData.from_date }) }} - {{ $t('report.label.toDate', { date: formData.to_date, title: t('sidebarMenu.report.expense') }) }}</h5>
            </div>

            <div class="mt-8 mb-0 ms-8" id="invoice-content">
                <h5 class="fs-3">{{ $t('buyer.label.totalExpense') }}: <span class="fw-normal">{{ props.totalAmount }}</span></h5>
            </div>
                ...
                <div class="card-title">
                    ...
                    <div class="d-flex align-items-center position-relative my-1">
                        ...
                        <input
                            ...
                            :placeholder="$t('report.placeholder.searchExpenses')"
                        />
                    </div>
                    <!--end::Search-->
                </div>
            ...
            <div class="card-body pt-0" id="invoice-content">
                ...
                >
                    ...
                    <template v-slot:invoice_number="{ row: expense }">
                        {{ expense.invoice_number }}
                    </template>

                    <template v-slot:expense_date="{ row: expense }">
                        <span v-if="!expense.is_total_row">{{ formatDate(expense.expense_date) }}</span>
                        <span v-else></span>
                    </template>
                    ...
                    <template v-slot:note="{ row: expense }">
                        <span v-if="!expense.is_total_row">{{ expense.note }}</span>
                        <strong v-else class="text-end">{{ $t('label.total') }}</strong>
                    </template>

                    <template v-slot:amount="{ row: expense }">
                        <strong v-if="expense.is_total_row">{{ expense.amount }}</strong>
...
const props = defineProps({
    ...
    expenses: Array as () => IExpense[] | undefined,
    totalAmount: Number,
    query: Object,
...
const tableHeader = ref([
    {
        columnName: t('expense.label.invoiceNumber'),
...
const initData = ref<IExpense[]>([]);

onMounted(() => {
    if(props.expenses){
        initData.value = props.expenses.map((expense: any)=>({
            id: expense.id,
...
//Calculate Summation
const getTotal = (field: keyof IExpense) => {
  return tableData.value.reduce((sum, row) => sum + Number(row[field] || 0), 0);
};

const computedExpenseTable = computed(() => {
...
const applyFilter = () => {
    Inertia.replace(route('reports.expense'), {
        preserveScroll: true,
        preserveState: true,
        data: {
...
const printInvoice = () => {
    document.title = `Expense Report ${formData.from_date} to ${formData.to_date}`;
    window.print();
};
</script>
