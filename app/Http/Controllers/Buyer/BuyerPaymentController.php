...
use App\Http\Requests\Buyer\CreateWaiverPaymentRequest;
use App\Http\Requests\Buyer\UpdateWaiverPaymentRequest;
...
class BuyerPaymentController extends Controller implements HasMiddleware
{
    ...
    public static function middleware(): array
    {
        return [
            ...
            new middleware('permission:can-edit-buyer-payment', only: ['update','updateWaiverPayment']),
            new middleware('permission:can-delete-buyer-payment', only: ['destroy']),
            new middleware('permission:can-add-buyer-waiver-payment', only: ['addWaiverPayment'])
        ];
    }
    ...
    public function update(UpdateBuyerPaymentRequest $request, Buyer $buyer, BuyerPayment $payment)
    {
        if (!$payment->is_editable) {
            return redirect()->back()->with('error', __('message.custom.payment.update.invalid'));
        }
        $validatedData = $request->validated();
    ...
    /* ----Waiver Payment----- */
    public function addWaiverPayment(CreateWaiverPaymentRequest $request, Buyer $buyer)
    {
        $validatedData = $request->validated();
        $buyerPayment = $this->buyerPaymentService->addWaiverPayment($buyer, $validatedData);
        $status = $buyerPayment ? 'success' : 'error';
        $message = $buyerPayment ? __('message.custom.payment.store.success') : __('message.custom.payment.store.error');
    ...
    public function updateWaiverPayment(UpdateWaiverPaymentRequest $request, Buyer $buyer, BuyerPayment $payment)
    {
        $validatedData = $request->validated();
        $buyerPaymentAccess = $this->buyerPaymentService->checkBuyerPaymentAccess($buyer, $payment);
        if($buyerPaymentAccess){
            $isUpdated = $this->buyerPaymentService->updateWaiverPayment($payment, $validatedData);
            $status = $isUpdated ? 'success' : 'error';
            $message = $isUpdated ? __('message.custom.payment.update.success') : __('message.custom.payment.update.error');
            return Inertia::location(route('buyers.payments.invoice', [
                'buyer' => $buyer->id,
