...
class SaleItemService extends BaseModelService
{
    ...
    public function createSaleItems(array $items, int $saleId)
    {
        ...
        foreach ($items as $saleItem) {
            ...
            $itemUnit = $this->itemUnitService->getItemUnitById($item->central_id);
    ...
    public function getSalesByBrand($request)
    {
        $from = $request->from_date ?? Carbon::today()->toDateString();
        $to = $request->to_date ?? Carbon::today()->toDateString();
        $brandId = $request->brand_id;

        $query = $this->model()::with('sale')
        ->whereHas('sale', function ($salesByBrand) use ($from, $to) {
            $salesByBrand->whereBetween('sale_date', [$from, $to]);
        });

        $salesByBrand = $query->where('brand_id', $brandId)->get();

        return [
            'salesByBrand' => $salesByBrand,
            'total_amount' => $salesByBrand->sum('total_price'),
        ];
    }
}
