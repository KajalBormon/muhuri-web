...
class ExpenseService extends BaseModelService
{
    ...
    {
        $invoiceNumber = HelperService::generateInvoiceNumber('E-', $this->model());
...
//        $expenses['total_expense'] = $totalExpense;
    ...
    public function addBuyerWaiverPaymentToExpense($waiverPayment, $expenseAbleType): void
    {
        $buyer = $waiverPayment->buyer()->first();
        $item = Item::where('slug', 'expense-others')->first();
        $data = $this->getExpenseDataFromObject($waiverPayment, $expenseAbleType);
        $data['item_id'] = $item->id;
        $data['is_editable'] = false;
        if (!$buyer->is_default) {
            $data['note'] = HelperService::getPaymentNote($waiverPayment);
        }
        $this->addExpense($data);
    }
    ...
    public function getExpensesDateAndTypeWise($request)
    {
        $from = $request->from_date ?? Carbon::today()->toDateString();
        $to = $request->to_date ?? Carbon::today()->toDateString();

        $query = $this->model()::whereBetween('expense_date', [$from, $to]);
        $expenses = $query->with('item')->get();
