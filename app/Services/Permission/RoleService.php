...
class RoleService extends BaseModelService
{
    ...
    public function syncData($request)
    {
        try {
            ...
            foreach ($roles as $role) {
                $role['central_id'] = $role['id'];
                $permissionIds = $role['permission_ids'] ?? [];
                $permissions = $this->getPermissionByIds($permissionIds);
    ...
    public function syncRoleDelete($data)
    {
        try{
            $role = $this->model()::where('central_id', $data['id'])->first();

            if ($role) {
                $role->permissions()->detach();
                $role->users()->detach();
                $role->name = $role->name.'-'.'deleted'.'-'.$role->id;
                $role->deleted_by = $data['deleted_by'];
                $role->save();
                $role->delete();
            }
        }catch(\Exception $e){
    ...
    public function getPermissionByIds($permissionIds)
    {
        return Permission::whereIn('central_id', $permissionIds)->get();
    }
}
